...
export default function FloatingTracker({ isVisible = true }: { isVisible?: boolean }) {
  ...
  return (
    <>
      <nav
        className={`fixed top-6 left-6 z-90 w-20 md:w-32 flex flex-col items-start gap-8 transition-all duration-500 ease-out ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
        {/* Top title */}
        <button
        ...
        >
          <h2 className="font-sans text-[0.60rem] tracking-[0.25em] text-[#1A1A1A]/40 group-hover:text-[#1A1A1A] group-active:text-[#1A1A1A] dark:text-white uppercase font-medium transition-colors duration-300">
            Zelarte Studio
          </h2>
        </button>
      ...
      <button
        ...
        className="fixed left-6 bottom-6 z-90 bg-white text-[#1A1A1A] border border-[#1A1A1A]/10 rounded-full p-2 text-[0.60rem] transition-all duration-300 shadow-lg dark:bg-[#111] dark:text-white dark:border-[#ffffff20] flex items-center justify-center"
      >
        {theme === 'light' ? (
